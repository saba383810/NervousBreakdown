...
const runtimeGuid = (0, import_utils.createGuid)();
function prepareGeneratedScript(source) {
  return source.replaceAll("$runtime_guid$", runtimeGuid).replace("kUtilityScriptIsUnderTest = false", `kUtilityScriptIsUnderTest = ${(0, import_utils.isUnderTest)()}`);
}
const kUtilityScriptSource = prepareGeneratedScript(rawUtilityScriptSource.source);
function accessUtilityScript() {
  return `globalThis['__playwright_utility_script__${runtimeGuid}']`;
}
const kPlaywrightBinding = "__playwright__binding__" + runtimeGuid;
