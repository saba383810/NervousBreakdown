...
class FFBrowserContext extends import_browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    const promises = [
      super._initialize(),
      this._browser.session.send("Browser.addBinding", { browserContextId: this._browserContextId, name: import_javascript.kPlaywrightBinding, script: "" }),
  ...
  async _updateInitScripts() {
    ...
    await this._browser.session.send("Browser.setInitScripts", { browserContextId: this._browserContextId, scripts: [import_page.kUtilityInitScript.source, ...bindingScripts, ...initScripts].map((script) => ({ script })) });
  }
